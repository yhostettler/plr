...
from plr_tasks.forward_patch_locomotion.managers.ema_manager import EMAManager
import plr_tasks.forward_patch_locomotion.mdp.markers as mdp_markers

if TYPE_CHECKING:
    from plr_tasks.forward_patch_locomotion.env_cfg import PLRLocomotionEnvCfg
...
class PLRLocomotionEnv(ManagerBasedRLEnv):
    ...
    - Binary map debug visualization: set cfg.viewer.debug_vis = True to draw
      forbidden cells of the shared binary map as red cubes in the Isaac Sim viewport.
    ...
    def step(self, action: torch.Tensor):
        ...
        if self._map_vis_markers is not None and hasattr(self, "plr_global_binary_map"):
            current_map = self.plr_global_binary_map  # (H, W) — shared across all envs
